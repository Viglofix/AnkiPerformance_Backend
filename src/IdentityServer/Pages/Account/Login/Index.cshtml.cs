...
using IdentityServer.Pages.Account.Login.Dtos;
        ...
        if (ModelState.IsValid)
        {
            ...
            if (result.Succeeded)
            {
                ...
                if (context != null)
                {
                    ...
                    // we can trust model.ReturnUrl since GetAuthorizationContextAsync returned non-null
                    // return Redirect(Input.ReturnUrl ?? "~/");
                    var client = new RestClient("http://localhost:5155/")
                        .AddDefaultHeaders(new()
                        {
                            {"x-csrf","1"}
                        })
                        .AddDefaultParameter("credentials", "include");

                    var request = new RestRequest($"ankiauthentication/login", Method.Post);
